...
class FullscreenPlayer(Gtk.Window):
    def __init__(self):
        ...
        # GStreamer pipeline. gtkglsink renders into a GTK widget while
        # glsinkbin performs conversion and scaling through OpenGL. This keeps
        # Gtk.Overlay labels above the video without the costly software
        # videoscale/videoconvert chain previously used with gtksink.
        self.pipe = Gst.ElementFactory.make("playbin", None)
        if not self.pipe:
            raise RuntimeError("Unable to create the GStreamer 'playbin' element.")

        self.video_widget = None
        self.using_overlay = False

        self.gtk_gl_sink = Gst.ElementFactory.make("gtkglsink", "gtk_gl_sink")
        self.gl_sink_bin = Gst.ElementFactory.make("glsinkbin", "gl_sink_bin")
        if not self.gtk_gl_sink or not self.gl_sink_bin:
            raise RuntimeError(
                "The GStreamer 'gtkglsink' and 'glsinkbin' elements are required. "
                "Install them with: sudo apt install gstreamer1.0-gl gstreamer1.0-gtk3"
            )

        if self.gtk_gl_sink.find_property("force-aspect-ratio"):
            self.gtk_gl_sink.set_property("force-aspect-ratio", False)

        self.gl_sink_bin.set_property("sink", self.gtk_gl_sink)
        self.pipe.set_property("video-sink", self.gl_sink_bin)

        self.video_widget = self.gtk_gl_sink.props.widget
    ...
    def enqueue_startup_sequence(self):
        ...
        # C10
        daymsg = os.path.join("/home/tme520/Videos/LPS/c10/announcements/FR/", f"c10 - good {WEEKDAY_NAMES[wd]}.mp4")
    ...
    def enqueue_day_greeting(self, now: Optional[datetime] = None):
        ...
        greeting_path = os.path.join(
            base_dir_nice, f"c18 - good {WEEKDAY_NAMES[weekday_idx]}.mp4"
        )
