...
class FullscreenPlayer(Gtk.Window):
    def __init__(self):
        ...
        if gtk_sink:
            if gtk_sink.find_property("force-aspect-ratio"):
                # Keep aspect ratio; letterbox/pillarbox will use widget background (we set it to white)
        ...
        /* Ensure the video widget (gtksink/drawingarea) paints white where letterboxing occurs */
        #video-area, drawingarea#video-area, widget#video-area {
    ...
    def _check_and_fire_scheduled(self, now: datetime):
        ...
        for idx, e in enumerate(self.schedule):
            ...
            if now.strftime('%Y-%m-%d %H:%M:%S') >= str(fire_dt):
