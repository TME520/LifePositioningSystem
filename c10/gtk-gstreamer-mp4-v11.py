...
def load_schedule() -> Tuple[List[ScheduleEntry], Dict[int, List[ScheduleEntry]]]:
    ...
    if not schedule_path:
        print("[Schedule] schedule.csv not found.")
...
def load_actions_script() -> Dict[str, List[Dict[str, str]]]:
    ...
    if not path:
        print("[Script] script.json not found.")
        return {}
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
            if not isinstance(data, dict):
                print("[Script] Invalid top-level JSON type.")
                return {}
            return data
    except Exception as ex:
        print(f"[Script] Failed reading script.json: {ex}")
        return {}
...
class FullscreenPlayer(Gtk.Window):
    ...
    def enqueue_file(self, path: str):
        if not path or not os.path.exists(path):
            print(f"[Queue] File not found, skipping: {path}")
            return
        if not self._playing:
            ...
            print(f"[Queue] Queued: {path} (queue length: {len(self.play_queue)})")
    ...
    def enqueue_hour_video(self, hour: int):
        ...
        if not path or not os.path.exists(path):
            print(f"[HourChange] Missing file for hour {hour:02d}: {path}")
            return False
