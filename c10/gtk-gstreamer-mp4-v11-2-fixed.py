...
class FullscreenPlayer(Gtk.Window):
    def __init__(self):
        ...
        if gtk_sink:
            ...
            # Give the video widget a stable CSS name for styling
            try:
        ...
        # Ensure the gtksink widget background switches with state
        # Default idle = black; playing = white
        # We rely on the 'window-idle' / 'window-playing' classes applied in code.
        .window-idle #video-area, .window-idle #video-area * {
            background-color: black;
        }
        .window-playing #video-area, .window-playing #video-area * {
